...
c inc -20 if c == 10`
	expectedHighest := 1
	expectedAllHighest := 10
	highest, allHighest := Day8Part1(input)

	if highest != expectedHighest {
		t.Errorf("Expected %v for highest, got %v for input %q", expectedHighest, highest, input)
	}

	if allHighest != expectedAllHighest {
		t.Errorf("Expected %v for all highest, got %v for input %q", expectedAllHighest, allHighest, input)
	}
}

func TestDay8Part1Input(t *testing.T) {
	input := day8Input
	expectedHighest := 4877
	expectedAllHighest := 5471
	highest, allHighest := Day8Part1(input)
...
const day8Input = `gug dec 188 if zpw >= 8
